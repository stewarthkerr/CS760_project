...
%Usage:
% [best_net,best_score]=globalMIT(a,alpha)
% Input:
...
function [best_net,best_score_ori]=globalMIT(a,alpha,allowSelfLoop)

if nargin<2 alpha=0.999;end;
if nargin<3 allowSelfLoop=1;end;  %allow self loop by default
...
Ne=n-1;  %#effective samples, assuming single time-series data
best_score_arr=zeros(1,dim);  %best modified S_{MIT} score for each individual node
best_score_ori_arr=zeros(1,dim);  %best original score for each individual node
HX_arr=zeros(1,dim);
best_net=zeros(dim,dim);
%main loop

fprintf('GlobalMIT Started, alpha= %f .....\n',alpha);
for i=1:dim  %search the best parent set for each variable independantly
    
    HX=myEntropy(a(2:end,i),Ne,n_state);
    HX_arr(i)=HX;
    ...
    for p=1:Pstar-1 %investigate all set from 1->P*-1 elements
        ...
        for j=1:nn   %loop through all potential parent set
            ...
            if nPa==1  %only canculate the score for the 1st level
                CMI=conditional_MI_DBN(a,Pa(1), i, 0,n_state);
                d_MIT=2*Ne*(HX-CMI);
                s_MIT=g_mit+d_MIT;
                ...
                CMI=conditional_MI_DBN(a,Pa(end), i, Pa(1:end-1),n_state);  %Attention: n-1 instead of n!
                score_i=score_i+CMI;
    ...
    fprintf('Best score %f - Original score %f. \n',best_s_MIT, 2*Ne*HX_arr(i)-best_s_MIT);
    best_Pa
    
    best_score_arr(i)=best_s_MIT;
    best_score_ori_arr(i)=2*Ne*HX_arr(i)-best_s_MIT;
