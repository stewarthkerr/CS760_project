...
int Factorial(int num);
	...
	float conditional_MI_DBN(int **data,int i, int j,int nPa,int *Pa, int n_state);
	float conditional_MI_DBN(int **data,int a, char b,int nPa, int* Pa, int n_state);
...
//////////////////Main program//////////////////////////////////////////////////
		...
		for(int p=1;p<Pstar;p++) { //loop through parent sets of increasing size
			...
			do  //generate all parent combination and score
			{
				...
				if (p==1){ //only canculate the score for the 1st level
					 float CMI=conditional_MI_DBN(data,cb[0], i, 0,cb,n_state);
					 ...
					 float CMI=conditional_MI_DBN(data,cb[p-1],i, p-1 ,cb,n_state);
					 score_i+=CMI;	
	...
	float best_score=0;
	for(int i=0;i<dim;i++) best_score+=2*(N-1)*HX_arr[i]-best_score_arr[i];
	cout<<"Best score =" << best_score<<endl;

	float score=getScore(net);
	cout<<"Best score again= "<<score<<endl;
...
void help(){
    using namespace std;
    cout << "Usage: globalMIT.exe <alpha> <inputFile> <outputFile> [options]"<<endl;
...
float getScore(int **net){
	...
	for(int i=0;i<dim;i++){
		...
		for(int j=0;j<nPa;j++){
			score_i+=2*(N-1)*conditional_MI_DBN(data, Pa[j],i,j,Pa,n_state)-chi[j+1];
		}
...
float conditional_MI_DBN(int **data,int a, int b,int nPa, int* Pa, int n_state){
float MI=0;
...
float conditional_MI_DBN(int **data,int a, int b,int nPa, char* Pa, int n_state){
float MI=0;
...
float myEntropy(int x){
	...
	for(int i=1;i<N;i++){// i run from 1
		H[data[i][x]]++;	
	}
	float e=0;
	for(int i=0;i<n_state;i++) {
